...
export function ChildrenPage() {
  ...
  const { children, currentChild, setCurrentChild } = useAppStore();
  ...
  const handleAdd = () => {
    ...
    useAppStore.getState().addChild(child);
    setShowAdd(false);
    setNewName('');
    setNewAge('');
  };
  ...
  return (
    <div className="min-h-screen pb-20">
      <Header title="儿童档案" showBack={false} right={
        <button onClick={() => navigate('/settings')} className="p-1 -mr-1 rounded-lg hover:bg-warm-100">
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h3 className="text-sm font-semibold text-warm-700">{child.name}</h3>
                <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${stageColors[child.stage]}`}>
                  {child.stage}
                </span>
              </div>
              <p className="text-xs text-warm-400">{child.age} 岁 · 第 {child.learningDay} 天学习</p>
            </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40">
          <div className="bg-white w-full max-w-mobile rounded-t-2xl sm:rounded-2xl p-6 space-y-4">
            <h3 className="text-base font-semibold text-warm-700">添加儿童档案</h3>
            <div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                onClick={handleAdd}
                className="flex-1 py-2.5 bg-brand-500 text-white text-sm rounded-xl"
              >
                确认
              </button>
            </div>
