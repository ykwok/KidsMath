...
import { mockWeeklyData, mockDailyReport, emotionConfig } from '@/data/mock';
...
export function ReportDailyPage() {
  const [date, setDate] = useState(mockDailyReport.date);
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="max-w-mobile mx-auto px-4 py-4 space-y-4">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-2xl font-bold text-warm-700">{report.duration} 分钟</p>
            </div>
            ...
            <div className={`px-2.5 py-1 rounded-full text-xs font-medium ${emotion.bg} ${emotion.color}`}>
        ...
        {/* Duration Chart */}
        <div className="card">
          <h3 className="text-sm font-semibold text-warm-700 mb-3">最近 7 天学习时长（分钟）</h3>
          <div className="h-48">
        ...
        {/* Accuracy Chart */}
        <div className="card">
          <h3 className="text-sm font-semibold text-warm-700 mb-3">最近 7 天正确率（%）</h3>
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
        ...
        {/* Levels List */}
        <div className="card">
          <h3 className="text-sm font-semibold text-warm-700 mb-3">今日完成关卡</h3>
          <div className="space-y-3">
            {report.levels.map((level) => {
              const levelEmotion = emotionConfig[level.emotion] || emotionConfig['开心'];
              return (
                <div key={level.id} className="flex items-center gap-3 p-3 bg-warm-50 rounded-xl">
                  <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-sm shadow-sm">
                    {levelEmotion.icon}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-warm-700 truncate">{level.title}</p>
                    <p className="text-[10px] text-warm-400">
                      用时 {formatDuration(level.duration)} · {level.correct}/{level.total} 正确
                    </p>
                  </div>
                  <div className={`text-[10px] px-2 py-0.5 rounded-full ${levelEmotion.bg} ${levelEmotion.color}`}>
                    {level.emotion}
                  </div>
                </div>
              );
