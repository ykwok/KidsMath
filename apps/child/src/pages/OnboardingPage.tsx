...
import { ChevronRight, Sparkles, User, Baby, Calendar, Send } from 'lucide-react';
import { useUserStore } from '@/store/userStore';
import type { OnboardingStep, AssessmentAnswer } from '@/types';

const ASSESSMENT_QUESTIONS = [
  { id: 1, question: '数一数，下面有几颗星星？', emoji: '⭐⭐⭐⭐⭐', options: ['3', '4', '5', '6'], correct: '5' },
  { id: 2, question: '3 个苹果加 2 个苹果等于几个？', emoji: '🍎🍎🍎 + 🍎🍎', options: ['4', '5', '6', '7'], correct: '5' },
  { id: 3, question: '5 比 3 多几个？', emoji: '5 - 3 = ?', options: ['1', '2', '3', '4'], correct: '2' },
  { id: 4, question: '把 4 分成两份，可以怎么分？', emoji: '4 = ?', options: ['1+2', '2+2', '3+1', '2+3'], correct: '2+2' },
  { id: 5, question: '数一数，下面有几根香蕉？', emoji: '🍌🍌🍌🍌🍌🍌🍌', options: ['6', '7', '8', '9'], correct: '7' },
];

export function OnboardingPage() {
  const navigate = useNavigate();
  const { setUser, setOnboardingStep, addAssessmentAnswer, completeOnboarding, onboardingStep } = useUserStore();

  const [role, setRole] = useState<'child' | 'parent'>('child');
  const [nickname, setNickname] = useState('');
  const [birthday, setBirthday] = useState('');
  const [assessmentIndex, setAssessmentIndex] = useState(0);
  const [assessmentResults, setAssessmentResults] = useState<AssessmentAnswer[]>([]);
  ...
  return (
    <div className="h-full flex flex-col px-6 pt-6 pb-4">
      ...
      <div className="flex gap-1 h-1 mb-6">
        {(['welcome', 'role', 'profile', 'assessment', 'passport', 'done'] as OnboardingStep[]).map((s, i) => (
          <div
            key={s}
            className={`flex-1 rounded-full ${
              i <= ['welcome', 'role', 'profile', 'assessment', 'passport', 'done'].indexOf(onboardingStep)
                ? 'bg-nebula-purple'
                : 'bg-white/10'
            }`}
          />
      ...
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence mode="wait">
          ...
          {onboardingStep === 'role' && (
            <RoleStep key="role" role={role} setRole={setRole} onNext={() => handleStep('profile')} />
          )}
      ...
      <div className="w-full max-w-xs space-y-4">
        <div>
          ...
          <div className="relative">
            <Sparkles size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
            <input
              type="text"
              value={nickname}
...
function AssessmentStep({ index, onAnswer }: { index: number; onAnswer: (a: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-xs bg-space-700/40 rounded-3xl p-5 mb-6">
        <h3 className="text-sm font-medium text-white/70 mb-4 text-center">能力雷达图</h3>
