...
export function PlanetPage() {
  ...
  const levels = LEVELS.filter((l) => l.module === module);
  const planetName = module === 'counting' ? '计数星云' : module === 'comparing' ? '比较黑洞' : '逻辑迷宫';
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide -mx-4 px-4">
          {levels.map((level, index) => {
            const status = levelStatuses.find((s) => s.levelId === level.id);
              ...
              >
                {isLocked ? (
                  <div className="h-80 rounded-3xl bg-space-700/40 border border-white/5 flex flex-col items-center justify-center gap-4">
                    ...
                    <p className="text-white/30 font-medium">关卡 {level.id}</p>
                  ...
                  <Link to={`/level/${level.id}`}>
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-4xl font-bold text-white/10">{String(level.id).padStart(2, '0')}</span>
                        {isCompleted && (
                          <div className="flex gap-0.5">
                            {Array.from({ length: status?.stars || 0 }).map((_, i) => (
                              <Star key={i} size={14} className="text-achievement-gold fill-achievement-gold" />
                            ))}
                          </div>
                        )}
                      </div>
                      ...
                      <p className="text-sm text-white/60 mb-auto">{level.description}</p>
